...
let pretty_number n = if n > 9 then "D" ^ string_of_int n else "D0" ^ string_of_int n
...
let create_year (conf : Auto_setup.Config.t) year =
  ...
  if file_exists "dune-project"
  then ()
  else (
    let ch = Out_channel.open_text "dune-project" in
    Out_channel.output_string ch (format_boilerplate conf year);
    Out_channel.close ch;
    let ch = Out_channel.open_text ".ocamlformat" in
    Out_channel.output_string ch "profile=janestreet";
    Out_channel.close ch)
;;

let handle_error ~bad ~good code =
  if code <> 0
  then Printf.printf "\o033[1m\o033[38;5;9m[error]\o033[0m\o033[38;5;8m %s\o033[0m\n" bad
  else
...
let do_start ?year ?day config =
  ...
  then ()
    ...
    Sys.command (Printf.sprintf "dune init exe %s %s --pu %s > /dev/null 2>&1" pd pd pd)
    |> handle_error ~bad:"cannot create day executable" ~good:("day " ^ pd ^ " created");
  Unix.chdir pd;
  Auto_setup.Input.download_input "input.txt" config.cookie year day
  |> handle_error ~bad:"cannot download input" ~good:"input downloaded";
...
let config_arg =
  ...
  Arg.(
    value
    & opt string "/home/lucas/.config/.aoc_as"
...
let setup_cmd =
  ...
  let term =
    Term.(
      const (fun config cookie path username github ->
        do_setup ~config ~cookie ~path ~username ~github)
      $ config_arg
      $ cookie_arg
      $ path_arg
      $ username_arg
...
let start_cmd =
  ...
  let term =
    Term.(
      const (fun config y d -> do_start ?year:y ?day:d config)
      $ config_arg
      $ year_arg
      $ day_arg)
  in
